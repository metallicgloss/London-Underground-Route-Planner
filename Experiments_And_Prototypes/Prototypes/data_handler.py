...
if __name__ == "__main__":
    ...
    for i in range(len(data.index)):
        if pd.isna(data["Line"].iloc[i]):
            ...
            if data["Line"].iloc[i-1] == data["Line"].iloc[i+1]:
                print("Changing index {} to {}".format(i,data["Line"].iloc[i+1]))
    ...
    def insert_connections(dataframe,S):
        for i in range(dataframe.size):
            origin = S.get_station_node_by_name(dataframe["Origin"].iloc[i])
            destination = S.get_station_node_by_name(dataframe["Destination"].iloc[i])
