...
class RoutePlanner:
    ...
    def get_route(self, starting_station_name: str, destination_station_name: str) -> list:
        ...
        while len(remaining_stations) != 0:
            ...
            not_none_stations = list(filter(lambda x: self._route_calculator[x]["SHORTEST_TIME"] is not None, remaining_stations))
            not_none_stations = sorted(not_none_stations, key=lambda x: self._route_calculator[x]["SHORTEST_TIME"])
            if not_none_stations == []:
                none_type_stations = list(filter(lambda x: self._route_calculator[x]["SHORTEST_TIME"] is None, remaining_stations))
                current_station_name = (not_none_stations + none_type_stations)[0]
            else:
            ...
            current_station = self._station_handler.get_station_node_by_name(current_station_name)
            current_station_shortest_time = self._route_calculator[current_station_name]["SHORTEST_TIME"]
            
            for connected_station in current_station.connected_stations:
                # Get quickest connection to specific node
                quickest_time_index = current_station.connected_stations[connected_station]["TIME_TO"].index(min(current_station.connected_stations[connected_station]["TIME_TO"]))
                quickest_time = current_station.connected_stations[connected_station]["TIME_TO"][quickest_time_index]
                quickest_train_line = current_station.connected_stations[connected_station]["TRAIN_LINE"][quickest_time_index]
                
                if current_station_shortest_time is None or self._route_calculator[connected_station]["SHORTEST_TIME"] is None:
                    self._route_calculator[connected_station]["SHORTEST_TIME"] = current_station_shortest_time + quickest_time
                    self._route_calculator[connected_station]["FROM_STATION"] = current_station_name
                    self._route_calculator[connected_station]["FROM_TRAIN_LINE"] = quickest_train_line
                    self._route_calculator[connected_station]["CURRENT_STATION"] = connected_station
                elif current_station_shortest_time + quickest_time < self._route_calculator[connected_station]["SHORTEST_TIME"]:
                    self._route_calculator[connected_station]["SHORTEST_TIME"] = current_station_shortest_time + quickest_time
                    self._route_calculator[connected_station]["FROM_STATION"] = current_station_name
                    self._route_calculator[connected_station]["FROM_TRAIN_LINE"] = quickest_train_line
                    self._route_calculator[connected_station]["CURRENT_STATION"] = connected_station
...
if __name__ == "__main__":
    ...
    S.get_station_node_by_name("A").add_station_connection(S.get_station_node_by_name("C"), 3, "RED")
    S.get_station_node_by_name("A").add_station_connection(S.get_station_node_by_name("D"), 4, "RED")
    S.get_station_node_by_name("A").add_station_connection(S.get_station_node_by_name("D"), 5, "BLUE")
    
    # Add connections from B
    S.get_station_node_by_name("B").add_station_connection(S.get_station_node_by_name("E"), 3, "BLUE")
    
    # Add connections from C
    S.get_station_node_by_name("C").add_station_connection(S.get_station_node_by_name("D"), 2, "BLUE")
    S.get_station_node_by_name("C").add_station_connection(S.get_station_node_by_name("B"), 14, "RED")
    S.get_station_node_by_name("C").add_station_connection(S.get_station_node_by_name("B"), 7, "BLUE")
    
    # Add connections from D
    S.get_station_node_by_name("D").add_station_connection(S.get_station_node_by_name("E"), 2, "RED")
    
    # Add connectiond from E
    # There is None
    R = RoutePlanner(S)
    print(R.get_route("A", "B"))