...
class DataCleaner:
        ...
        def console_print_log(self) -> None:
            ...
            for log_type in self._log_records:
                if self._log_records[log_type] != []:
                    print(log_type + " (" + str(len(self._log_records[log_type])) + ")")
        ...
        for data in [train_line, location_a, location_b]:
            if data != "":
                ...
                if self._check_for_illegal_characters_present(data):
                    self._log_records.create_log(self._log_records.ILLEGAL_CHARACTER, "Illegal characters found in the text '" + data + "' on row " + str(row_number + 1))
            else:
                data = None
    ...
    def _get_train_information_from_excel(self):
        ...
        for row_number in range(self._LAST_ROW_NUMBER - 1):
            train_line, starting_point, next_point, time = self._get_row_information_from_excel(row_number)
            if train_line is None:
                self._log_records.create_log(self._log_records.MISSING_TRAIN_LINE, "Train line not provided on row " + str(row_number + 1))
            else:
                if train_line not in self._train_information:
                    self._train_information[train_line] = {}
                
                if starting_point is None:
                    self._log_records.create_log(self._log_records.MISSING_START_STOP, "Start location not provided on row " + str(row_number + 1))
                else:
                    ...
                    if next_point is not None:
                        ...
                        if time is not None:
                            self._train_information[train_line][starting_point][next_point].append(time)
                        else:
                            self._log_records.create_log(self._log_records.MISSING_TIME_ENTRY, "Missing time on row " + str(row_number + 1) +
                                                        " between " + starting_point + " and " + next_point + " using the " + train_line + " train line")
    ...
    def _scan_for_errors(self):
        ...
        self._check_for_duplicate_next_points_in_same_train_line() # TODO Find a way to determine if the node is meant to have multiple stations heading towards it e.g Harrow-on-the-Hill
            ...
            for starting_point in self._train_information[train_line]:
                for next_stop in self._train_information[train_line][starting_point]:
                    if next_stop not in used_destination_points:
                        ...
                        log_message = "Duplicate Destination route to " + next_stop + " using the " + train_line + " train line"
                        if log_message not in self._log_records.log_records[self._log_records.DUPLICATE_ROUTE]:
                            self._log_records.create_log(self._log_records.DUPLICATE_ROUTE, log_message) 
            ...
            for start_point in self._train_information[train_line]:
                for next_point in self._train_information[train_line][start_point]:
                    if len(self._train_information[train_line][start_point][next_point]) > 1:
                        self._log_records.create_log(self._log_records.MULTIPLE_TIME_ENTRIE, 
                                                     "Multiple times provided from " + start_point + " to " + next_point + 
                                                     " using the " + train_line + " train line: [" + 
                                                     ",".join([str(x) for x in self._train_information[train_line][start_point][next_point]]) + "]")
                    elif len(self._train_information[train_line][start_point][next_point]) == 0:
                        self._log_records.create_log(self._log_records.MISSING_TIME_ENTRY, 
                                                     "No time entry provided from " + start_point + " to " + next_point + " using the " + train_line + " train line")
                    for time in self._train_information[train_line][start_point][next_point]:
                        if time >= upperbound or time <= lower_bound or time == 0:
                            self._log_records.create_log(self._log_records.EXTREME_VALUE, "Extreme value '" + str(time) + 
                                              "' detected from "+  start_point + " to " + next_point + " using the " + train_line + " train line")
    ...
    def _check_for_missing_routes(self):
        ...
        for train_line in self._train_information:
            all_stations_used_frequencies = {a:0 for a in self._train_information[train_line]}
            for starting_point in self._train_information[train_line]:
                for next_point in self._train_information[train_line][starting_point]:
                    if next_point not in list(all_stations_used_frequencies.keys()):
                        self._log_records.create_log(self._log_records.UNKOWN_STATION_NAME,
                                                      "Station " + next_point + " has been registred as a station for the " + train_line + " train line")
                    else:
                        all_stations_used_frequencies[next_point] += 1
            for starting_point in all_stations_used_frequencies:
                starting_point_has_not_been_visited = all_stations_used_frequencies[starting_point] == 0
                ...
                if starting_point_has_not_been_visited and starting_point_has_no_next_stop:
                    self._log_records.create_log(self._log_records.MISSING_ROUTE,
    ...
    def _check_for_missing_end_nodes(self):
        ...
        for train_line in self._train_information:
            last_stop_key = list(self._train_information[train_line].keys())[-1]
            if self._train_information[train_line][last_stop_key] != {}:
                self._log_records.create_log(self._log_records.MISSING_END_NODE,
...
if __name__ == "__main__":
    LAST_ROW_NUMBER = 758
    data_cleaner_obj = DataCleaner("London Underground data.xlsx", LAST_ROW_NUMBER)
